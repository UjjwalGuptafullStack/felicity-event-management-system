...
export const OrganizerNav = () => {
  ...
  const getTitle = () => {
    if (location.pathname.includes('/events/') && location.pathname.includes('/detail')) return 'Event Details';
    if (location.pathname.includes('/attendance/')) return 'Attendance Scanner';
    if (location.pathname.includes('/merchandise-approvals')) return 'Merchandise Approvals';
    
    const currentItem = navItems.find(item => item.path === location.pathname);
    return currentItem ? currentItem.label : 'Organizer Portal';
  };
